...
% i=i+1;
% alg(i).name='coherent';
% alg(i).star_list=[1 5:5:35]; %1:V/2;
% for m=1:length(alg(i).star_list)
%     alg(i).edge_list{m}=unique(round(logspace(0,log10(V*alg(i).star_list(m)/2),100))); %1:V*alg(i).star_list(m)/2;
% end
...
xmax=1000;
h(1)=subplot(321);
semilogx(L(2).Lhat,'color','k','linewidth',2)
...
h(4)=subplot(324);
imagesc(L(3).Lhat(min(I):max(I),min(J):max(J))), colorbar
xlabel('size of signal subgraph','fontsize',fs)
ylabel('# star-vertices','fontsize',fs)
