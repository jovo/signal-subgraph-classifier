...
We consider up to two additional constraints on $\mc{S}$.  First, the size of the signal subgraph may be constrained such that $|\mc{S}| \leq s$. Second, the minimum number of vertices onto which each edge is incident is constrained such that $\mc{S}=\{(u,v): u \in \mc{M} \cup v \in \mc{M}\}$, where $\mc{M}$ is a set of \emph{signal vertices} with $|\mc{M}|\leq m$. 
