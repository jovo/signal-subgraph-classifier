...
\author{Joshua~T.~Vogelstein, %,~\IEEEmembership{Member,~IEEE,}
		Henry Pao
        R.\ Jacob~Vogelstein, %John~Doe,~\IEEEmembership{Fellow,~OSA,}
        and~Carey~E.~Priebe %Jane~Doe,~\IEEEmembership{Life~Fellow,~IEEE}% <-this % stops a space
...
\subsubsection{Identical and independent edge model} % (fold)
\label{sub:ER}
...
\begin{align}
	\PP_{\thet}[G]&=\PP_{\thet}[\mvb{A}]=\prod_{i,j \in [n]} \PP_{\thet}[A_{ij}] %= \prod_{ij} \PP[A_{ij} | p^Y]
...
\subsubsection{Incoherent model} % (fold)
...
% subsubsection incoherent_subspace (end)

\subsubsection{Star$_1$  model} % (fold)
\label{ssub:incoherent_subspace}
...
\subsection{Model estimates} % (fold)
...
% Nonetheless, when $m$ is known in either the incoherent $q$ model or the cross $q$ model, estimating $q$ and $\mc{M}_i$ or $\mc{M}_*$ is straightforward.  First, obtain estimates $\{\mh{p}_{ij}\}$.  Second, rank them, $\mh{p}_{(1)}, \ldots, \mh{p}_{(d)}$.  Since we know that $p<q$, average the first $d-m$ order parameters to obtain $\mh{p}$, and the last $m$ order parameters to obtain $\mh{q}$.  Finally, let $\mc{M}_i$ or $\mc{M}_*$ be the set of vertices with the largest $m$ order parameters.  



% \subsubsection{Erd\"os R\'enyi random graph model parameter estimate} % (fold)
...
% \end{align}
% where $\mh{L}_{\mhc{E}}$ is the average misclassification rate on held-out data as defined in \eqref{eq:L2}, so as to ensure $\mhc{E}$ is not overfit. And note that we do not 
% 
...
\begin{figure}[h!]
\centering \includegraphics[width=.9\linewidth]{/Users/jovo/Research/figs/graph_sims/subgraph_types}
...
\subsubsection{Star$_1$ signal subgraph search} % (fold)
...
\subsection{Monotonicity of error given $T$} % (fold)
...
\subsection{Approximate Asymptotica distribution of $T$} % (fold)
...
\subsection{Simulated classification results} % (fold)
\label{sub:classification_results}

% subsection classification_results (end)


\subsection{Connectome classification results} % (fold)
